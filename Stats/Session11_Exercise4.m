...
indt = ttest2(difficult,easy)

difficultm = mean(difficult);
difficultsumofsq = sum(difficult.^2)-sum(difficult)^2/length(difficult);

easym = mean(easy);
easysumofsq = sum(easy.^2)-sum(easy)^2/length(easy);

sp_squared = (difficultsumofsq+easysumofsq)/(length(easy)+length(difficult)-2);
s = sqrt(sp_squared/length(difficult)+sp_squared/length(easy));
t = ((difficultm-easym)-0)*1/s
...
dept = ttest(CPaired,CcontrolPaired)
%Step 1 - work out d and dbar
d = CPaired-CcontrolPaired;
Dbar = sum(d)/length(d);

%step 2 - Sum of squared differences
ssd = sum(d.^2) - sum(d)^2/length(d);

%Step 3 - Squrare root by the degrees of freedom
sd = sqrt(ssd/(length(d)-1));

%Step 4 - estimated standard error
